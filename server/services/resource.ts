...
class ResourceService implements IResService {
  ...
  find = async (projection: Object): Promise<any> => {
    const foundRes = await ResourceModel.find(projection).lean();

    return foundRes;
  };

  findById = async (id: Types.ObjectId): Promise<any> => {
    const foundRes = await ResourceModel.findById(id).lean();

    return foundRes;
  };

  findOne = async (projection: Object): Promise<any> => {
    const foundRes = await ResourceModel.findOne(projection).lean();
  ...
  update = async (r: IResource): Promise<any> => {
    const rUpdated = await ResourceModel.findOneAndUpdate({ _id: r._id }, r, {
      new: true,
    });
  ...
  findOrCreate = async (resource: IResource): Promise<any> => {
    const found = await ResourceModel.findOne({
