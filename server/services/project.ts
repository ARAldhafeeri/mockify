...
class ProjectService implements IProjectService {
  ...
  find = async (projection: Object): Promise<any> => {
    const foundProject = await ProjectModel.find(projection);
  ...
  update = async (p: IProject): Promise<any> => {
    const pUpdated = await ProjectModel.findOneAndUpdate({ _id: p._id }, p, {
      new: true,
    });
  ...
  findOne = async (projection: Object): Promise<any> => {
    const foundRes = await ProjectModel.findOne(projection).lean();
