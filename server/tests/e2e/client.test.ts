...
describe('end-to-end tests project endpoint', () => {
  ...
  test('should create client', async () => {
    ...
    const response = await request.agent(app).post(`${API_ROUTE}${CLIENT_ROUTE}`).send({
      ...mockData
    })
  ...
  test('should get clients related to user projects', async () => {
    ...
    const response = await request.agent(app).get(`${API_ROUTE}${CLIENT_ROUTE}?projectId=${projectId}`)
    .set('Authorization', 'bearer ' + token)
  ...
  test('should edit client', async () => {
    ...
    const response = await request.agent(app).put(`${API_ROUTE}${CLIENT_ROUTE}`).send({
  ...
  test('should delete client', async () => {
    const response = await request.agent(app).delete(`${API_ROUTE}${CLIENT_ROUTE}/?id=${created._id}`)
    .set('Authorization', 'bearer ' + token)
    expect(response.status).toBe(200);
    expect(response.body.status).toBe(true);
  });
